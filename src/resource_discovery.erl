...
start() ->
    application:start(rd_log),
    rd_log:info("starting resource_discovery..."),
...
sync_resources(Timeout) ->
    ...
    Self = self(),
    Nodes = nodes(known),
    rd_log:info("synching resources to nodes: ~p", [Nodes]),
...
ping_contact_nodes([], _Timeout) ->
    rd_log:info("No contact node specified. Potentially running in a standalone node", []),
...
ping_contact_nodes(Nodes, Timeout) ->
    Reply = rd_util:do_until(fun(Node) ->
				 ...
				 pong -> true;
				 pang ->
				     rd_log:info("ping contact node at ~p failed", [Node]), 
				     false
			     end
		     end,
		     Nodes),
...
rpc_call(Type, Module, Function, Args, Timeout) ->
	...
	{ok, Resource} -> 
	    rd_log:info("got a resource ~p", [Resource]),
	    case rpc:call(Resource, Module, Function, Args, Timeout) of
		{badrpc, Reason} ->
		    rd_log:info("got a badrpc ~p", [Reason]),
		    delete_resource_tuple({Type, Resource}),
		    rpc_call(Type, Module, Function, Args, Timeout);
		Reply ->
		    rd_log:info("result of rpc was ~p", [Reply]),
		    Reply
	    end;
...
rpc_multicall(Type, Module, Function, Args, Timeout) ->
    case get_resources(Type) of
        [] -> {error, no_resources};
	Resources -> 
	    rd_log:info("got resources ~p", [Resources]),
